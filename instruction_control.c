...
//OP CODES
Instruction_Control opcode_flags[64] = {
...
void execute_instruction(CPU *cpu, uint32_t inst) { 
    Instruction decoded_inst = decode(inst); 
    if (cpu->instruction_table[decoded_inst.opcode]) { 
        cpu->instruction_table[decoded_inst.opcode](cpu); 
    } else { instruction_exception(cpu); 
