...
void EX_stage(CPU *cpu) {
    ...
    if(cpu->pipeline.RFEX_READ.control.type == I_TYPE) {
        cpu->pipeline.EXDC_WRITE.Write_Reg_Num = cpu->pipeline.RFEX_READ.instruction.rt;
    } else if (cpu->pipeline.RFEX_READ.control.type == R_TYPE){
...
void DC_stage(CPU *cpu) {
    ...
    if(cpu->pipeline.EXDC_READ.control.memAccess == NO_MEM){
...
void WB_stage(CPU *cpu) {
    ...
    if(cpu->pipeline.DCWB_WRITE.control.f_type == R_TYPE) {
        cpu->gpr[cpu->pipeline.DCWB_WRITE.instruction.rs] = cpu->pipeline.DCWB_WRITE.ALU_Result;
    } else if(cpu->pipeline.DCWB_WRITE.control.type == COPROC && cpu->pipeline.DCWB_WRITE.control.f_type == LOAD) {
        cpu->cp0.gpr[cpu->pipeline.DCWB_WRITE.Write_Reg_Num] = cpu->pipeline.DCWB_WRITE.ALU_Result;
    }

    uint64_t result = cpu->pipeline.EXDC_WRITE.ALU_Result;
    int writeReg = cpu->pipeline.EXDC_WRITE.Write_Reg_Num;
...
void MTC0(CPU *cpu) {
    ...
    uint8_t write_reg = cpu->pipeline.RFEX_READ.instruction.rd;
}
